...
public class ChallengeBookStore {
    public static void main(String[] args) {
        ...
        books.stream().map(book -> "Coming Soon " + book.title).forEach(System.out::println);

        System.out.println("\n Display the books sorted by their prices in ascending order.");
        books.stream().sorted(Comparator.comparing(book -> book.price)).forEach(System.out::println);

        System.out.println("\n A rare book is one where the title has more than 10" + " characters.");
        books.stream().filter(book -> book.title.length() >= 10).forEach(System.out::println);
